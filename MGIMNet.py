...
##########################################################################
##---------- MGIMNet-LOL ----------
class MGIMNet(nn.Module):
    def __init__(self, in_chn=3, wf=64, depth=4):
        super(MGIMNet, self).__init__()
        self.fc1 = nn.Linear(4000, 2000)
        self.fc2 = nn.Linear(2000, 4096)
        ...
        # self.last1 = CALayer(2,reduction=2)
        # self.last2 = conv3x3(2, 1, bias=True)
    ...
    def forward(self, x):
        ...
        # a=self.last1(img)
        # print(a.shape)
        # out_1 = self.last(msff_result)+self.last2(self.last1(img))
...
#     input = torch.ones(2,4000, dtype=torch.float, requires_grad=False).cuda()
#
#     model = MGIMNet(in_chn=1, wf=96, depth=4).cuda()
#     out = model(input)
